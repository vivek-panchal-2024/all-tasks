...
const registerUser = async (req, res) => {
  try {
    ...
    if (isUserExist.length > 0) {
      return res.render("./login/RegistrationResponse", {
        auth_code: "",
        link: "",
      ...
      res.render("./login/RegistrationResponse", {
        auth_code,
        link: `http://localhost:3000/activate-user/${auth_code}`,
        error: "",
      });
    }
  } catch (error) {
...
const activateUser = async (req, res) => {
  try {
    ...
    if (isActivate[0].activate_status === 1) {
      return res.render("./login/ActivateView", {
        error: "Link is expired.",
        message: "",
      });
    } else if (
      isActivate[0].activate_status === 0 &&
      (currentDate - createdAt) / 60000 > 60
    ) {
      ...
      if (deleteUnactivateUser.affectedRows !== 0) {
        return res.render("./login/ActivateView", {
          error: "Link is expired. Now Register again.",
          message: "",
        });
      } else {
        return res.render("./login/ActivateView", {
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (req.body.email) {
      ...
      if (getUser.length === 1) {
        return res.render("./login/ResetPwd", {
          auth_code: getUser[0].auth_code,
          error: "",
          successMessage: "",
        });
      } else {
      ...
      if (getUser.length === 1) {
        ...
        if (
          resetPassword.changedRows !== 0 ||
          resetPassword.affectedRows !== 0
        ) {
          return res.render("./login/ResetPwd", {
            auth_code: "",
            error: "",
