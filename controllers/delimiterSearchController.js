...
const renderSearchView = (req, res) => {
  if (db) {
    res.render("./task9/StudentSearch", {
      result: "",
      fields: "",
      error: "",
      query: "",
...
const getSearchResult = (req, res) => {
  if (db.connection) {
    ...
    for (let i = 0; i <= input.length; i++) {
      if (
        delimeterObject.hasOwnProperty(input.charAt(i)) ||
        i === input.length
      ) {
        if (inputValue !== "" && delimeter !== "") {
          if (inputObject[delimeterObject[delimeter]]) {
            inputObject[
              delimeterObject[delimeter]
            ] += ` OR ${delimeterObject[delimeter]} LIKE '%${inputValue}%'`;
          } else {
    ...
    if (Object.keys(inputObject).length === 0) {
      return res.render("./task9/StudentSearch", {
        result: "",
        fields: "",
        error: "Please Enter the field value",
        query: "",
      });
    } else {
