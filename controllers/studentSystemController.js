    ...
    if(db.connection){
        ...
        db.connection.query(sql, [1, 2], (error, result)=>{ 
            ...
            if(req.query.pg && req.query.attendance){
                res.render("./task8/StudentAttendance", {result: result[0], days: days, attendance: req.query.attendance, pg: req.query.pg, url: {1: req.url.split("?")[0], 2: "&", 3: req.url.split(`${req.query.pg}&`)[1]}, lastPageNumber: lastPageNumber, fieldName: fieldName, type: ordered_type})
            } else if(req.query.pg){
                res.render("./task8/StudentAttendance", {result: result[0], days: days, attendance: 1, pg: req.query.pg, url: {1: req.url.split("?")[0], 2: "&", 3: req.url.split(`${req.query.pg}&`)[1]}, lastPageNumber: lastPageNumber, fieldName: fieldName, type: ordered_type});
            } else{
                res.render("./task8/StudentAttendance", {result: result[0], days: days, attendance: 1, pg: 1, url: {1: req.url.split("?")[0]}, lastPageNumber: lastPageNumber, fieldName: fieldName, type: ordered_type});
            }
        });
    } else{
    ...
    if(db.connection){
        if(req.query.attendance === "1" || req.query.attendance === undefined ){
            ...
            if(student_id){
                ...
                if(student_name && student_present_days && student_percentage && student_percentage !== "0"){                    
                    // console.log("2");
        ...
        db.connection.query(sql, (error, result)=>{
            if(result){
                res.render("./task8/StudentAttendance", {result, days: days, attendance: req.query.attendance, fieldName: "", type: "", pg: 1, url: {1: req.url.split("?")[0], 2: "&", 3: req.url.split(`&`)[1]}, lastPageNumber: 1});   
            } 
        })
    } else{
...
const getResult = (req, res)=>{
    ...
    if(db.connection){
        db.connection.query(sql, [1,2,3,4], (error, result)=>{
            ...
            if(req.query.pg){
                res.render("./task8/StudentResult",{ result, exam_one: result[0], exam_two: result[1], exam_three: result[2], url: {1: req.url.split("?")[0]}, pg: req.query.pg, lastPageNumber: lastPageNumber} );
            } else{
                res.render("./task8/StudentResult",{ result, exam_one: result[0], exam_two: result[1], exam_three: result[2], url: {1: req.url}, pg: 1, lastPageNumber: lastPageNumber} );
            }
        });
    } else{
        res.send("Something went wrong");
...
const getReportCard = (req, res)=>{
    ...
    if(db.connection){
        ...
        db.connection.query(sql, [1,2,3, 4] , (error, result)=>{
            res.render("./task8/StudentReportCard", {exam_one: result[0], exam_two: result[1], exam_three: result[2], attendance: result[3]});
        });
