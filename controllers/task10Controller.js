  ...
  try {
    if(req.query.student){
      ...
      if(result[0].length === 0 && result[1].length === 0 && result[2].length === 0 && result[3].length === 0 && result[4].length === 0 && result[5].length === 0 && result[6].length === 0){
        res.render("./task10/JobApplicationForm", {basicDetails: "", educationDetails: "", workExperience: "", language: "", technology: "", reference: "", preferences: "", student: "", error: "No Data Found"});
      } else{
        res.render("./task10/JobApplicationForm", {basicDetails: result[0], educationDetails: result[1], workExperience: result[2], language: result[3], technology: result[4], reference: result[5], preferences: result[6], student: id, error: ""});
      }

    } else{
      res.render("./task10/JobApplicationForm", {basicDetails: "", educationDetails: "", workExperience: "", language: "", technology: "", reference: "", preferences: "", student: "", error: ""});
    }
  } catch (error) {
    console.error(error);
  }
...
const postJobApplication = async (req, res) => {
  try {
    ...
    if(student !== ""){
      ...
      educationData.filter( education =>{
        return education["university"] !== "" && education["passingYear"] !== "" && education["percentage"] !== "" && education["courseName"] !== "";
      }).forEach( async (education) =>{
