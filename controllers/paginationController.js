...
const getStudents = (req, res) => {
  ...
  if (db.connection) {
    ...
    if (req.query.fieldName && req.query.ordered_type) {
      ...
      sql =
        "select FirstName, LastName, Gender, DOB, ContactNumber, College_Name, Department, City, State, Hobby from student_26thFeb ORDER BY " +
        fieldName +
        " " +
        ordered_type +
        ` limit 200 OFFSET  ${offsetValue}`;
    } else if (
      (req.query.pg && req.query.fieldName == "") ||
      req.query.ordered_type == ""
    ) {
      fieldName = "FirstName";
      ordered_type = "asc";
      sql =
    ...
    db.connection.query(sql, (error, result) => {
      ...
      if (req.query.pg && req.query.fieldName && req.query.ordered_type) {
        res.render("./task7/StudentRecord", {
          result,
          number: `${offsetValue + 1}`,
          pg: req.query.pg,
          order: 1,
          field: fieldName,
          type: ordered_type,
...
const generateGrid = (req, res) => {
  try {
    if (inputQuery != "" && inputQuery.split("limit")[1] === undefined) {
      ...
      let formattedQuery =
        `${inputQuery.split(";")[0]} limit ${offsetValue}, 50;` + inputQuery;
